...
import {ApiService, TokenService} from '../service';
import {Router} from '@angular/router';
import {ApiResponse, APIURIPublic, Token} from '../data';
import {AddTokenHeaderFn, HttpInterceptorCommonErrorHandlerFn, HttpInterceptorHandlerFn} from '../data/type';
import {publicRoutes} from '../data/enum/url.enum';
import {AppNode} from '../../ui/app.node';
  ...
  if (err.status === 401 || err.status === 403) {
    ...
    if (!tokenService.token$().isEmpty) {
      return api.post(APIURIPublic.REFRESH, { refresh: tokenService.token$().refreshToken })
        .pipe(switchMap((result: ApiResponse) => {
          if (result.result) {
